...
public class CatalogCoursesPage extends AbsBasePage {
    ...
    @DisplayName("Объщий класс для слайдеров")
    ...
    @DisplayName("Сплитом обрезаю и беру только дату и месяц")
    ...
    @DisplayName("Сплитом обрезаю и беру только дату")
    private int parseDuration(String durationText) {
        String[] words = durationText.split(" ");
        return Integer.parseInt(words[0]);
    }

    @DisplayName("Проверка, что список курсов изменился")
    ...
    @DisplayName("Проверка, что при сбросе фильтров отображаются дефолтные курсы")
